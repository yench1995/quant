...
from ..models.backtest import BacktestRun, BacktestResult, Trade
...
class BacktestEngine:
    ...
    async def run(self, run_id: str):
        ...
        async with AsyncSessionLocal() as session:
            ...
            try:
                await self._execute(session, run)
    ...
    async def _execute(self, session: AsyncSession, run: BacktestRun):
        ...
        cache_mgr = CacheManager(session)
        ...
        # ── Step 1: LHB data — query DB first, fetch missing dates if needed ──
        lhb_df_all = await self._get_lhb_data(session, run.start_date, run.end_date, trading_days)

        # Build a per-date lookup dict: date -> DataFrame slice
        ...
        # ── Step 3: Price data — query DB first, fetch missing symbols ──
        symbols = list({sig.symbol for _, _, _, sig in all_signals})
        price_cache = await self._get_price_data(session, symbols, run.start_date, run.end_date)
    ...
    async def _get_lhb_data(
        self, session: AsyncSession, start_date: str, end_date: str, trading_days: list[str]
    ) -> pd.DataFrame | None:
        """Get LHB data from DB first; fetch network only for missing trading days."""
        result = await session.execute(
            select(LHBDaily).where(
                LHBDaily.date >= start_date,
                LHBDaily.date <= end_date,
            )
        )
    ...
    async def _get_price_data(
        self, session: AsyncSession, symbols: list[str], start_date: str, end_date: str
        ...
        result = await session.execute(
            select(StockPriceDaily).where(
                StockPriceDaily.symbol.in_(symbols),
                StockPriceDaily.date >= start_date,
                StockPriceDaily.date <= end_ext,
            )
        )
        rows = result.scalars().all()
        ...
        if missing_symbols:
            ...
            await asyncio.gather(*[fetch_one(s) for s in missing_symbols])
